...
public class CourseNInstructorConn {
    ...
    public ArrayList<CourseNInstructorModel> checkInstructorsRecords(String courseId){
        ...
        try {
            MyDB = helper.getReadableDatabase();
            c = MyDB.rawQuery("SELECT course.course_name, instructor.name FROM course, instructor JOIN course_instructors ON course.course_number = course_instructors.course_id AND course_instructors.course_id= ? AND instructor.id = course_instructors.instructor_id", new String[]{courseId});
    ...
    public ArrayList<CourseNInstructorModel> getCourseInstructorsRecords(String courseName){
        ArrayList<CourseNInstructorModel> records = new ArrayList<>();

        Cursor c = null;
        try {
            MyDB = helper.getReadableDatabase();
            c = MyDB.rawQuery("SELECT course.course_name, course.max_numb_students, instructor.name FROM course, instructor WHERE course.course_name = ?", new String[]{courseName});
